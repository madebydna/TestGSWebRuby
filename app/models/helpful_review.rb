class HelpfulReview < ActiveRecord::Base
  ...
  scope :filter_by_active, -> { where("active = 1") }

  def self.helpful_counts(reviews)
    helpful_count = HelpfulReview.select("review_id, Count(*) as count").in_review_list(reviews).filter_by_active.group(:review_id)
    helpful_count.inject({}) { |hash, row| hash[row[:review_id]] = row[:count]; hash }
  end

  def self.helpful_counts_by_id(review_ids)
    helpful_count = HelpfulReview.select("review_id, Count(*) as count").in_review_list_id(review_ids).filter_by_active.group(:review_id)
    helpful_count.inject({}) { |hash, row| hash[row[:review_id]] = row[:count]; hash }
  end
