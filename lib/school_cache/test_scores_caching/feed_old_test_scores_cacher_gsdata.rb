...
class TestScoresCaching::FeedOldTestScoresCacherGsdata < TestScoresCaching::TestScoresCacherGsdata
  CACHE_KEY = 'feed_old_test_scores_gsdata'
  HASH_NAME_MAP = {
      school_value: 'score',
      source_date_valid: 'year',
      proficiency_band_name: 'proficiency-band-name',
      proficiency_band_id: 'proficiency-band-id',
      data_type_abbr: 'test-abbr',
      data_type_id: 'test-id',
      breakdown_ids: 'breakdown-id',
      composite_of_pro_null: 'composite-of-pro-null',
  ...
  def result_to_hash(result)
    ...
    {}.tap do |h|
      h[:data_type] = result.name  #data_type.name
      h[:data_type_abbr] = result.short_name  #data_type.short_name
      h[:data_type_id] = result.data_type_id  #data_type.id
      h[:description] = result.description # source.description
...
# rubocop:enable Style/SafeNavigation
  ...
  def cache_exceptions
    %i(data_type percentage narrative label methodology source_year source_name breakdown_tags flags district_value state_value state_cohort_count)
  end
end