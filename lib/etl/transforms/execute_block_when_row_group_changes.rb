...
class ExecuteBlockWhenRowGroupChanges < GS::ETL::Source
  ...
  def initialize(group_by_fields, options = {}, &key_change_callback)
    ...
    @pass_rows_through_on_key_same = true unless options.has_key?(:pass_rows_through_on_key_same)
    @pass_rows_through_on_key_change = true unless options.has_key?(:pass_rows_through_on_key_change)
  ...
  def process(row)
    ...
    if @last_key && key != @last_key
      ...
      @last_key = key
      return @key_same_callback.call(@rows_in_group, row)
    end
  end
end
