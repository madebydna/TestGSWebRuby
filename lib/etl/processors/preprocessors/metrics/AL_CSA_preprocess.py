...
df_ACTCollegeReady = pd.read_excel(
    "2017-2018 College and Career Readiness Rate-edited2.xlsx",
    converters={"System Code": str, "School Code": str},
)
df_GradRate = pd.read_excel(
    "2017-2018 Graduation Rate-edited2.xlsx",
    converters={"System Code": str, "School Code": str},
)
df_CollegeEnrollment = pd.read_excel(
    "tabula-2017-2018 National Student Clearinghouse, College Going Rate Report-edited2.xlsx"
)
df_CollegeRemediation = pd.read_excel(
    "tabula-Alabama remediation-edited2.xlsx", converters={"state_id": str}
)
...
school_merged_CollegeEnrollment = df_CollegeEnrollment.merge(
    right=school_crosswalk_file,
    how="left",
    left_on="entity_name",
    right_on="entity_name",
)
district_merged_CollegeEnrollment = school_merged_CollegeEnrollment.merge(
...
transposed_CollegeEnrollment = pd.melt(
    district_merged_CollegeEnrollment,
    id_vars=[
        "entity_name",
        "entity_level",
        "cohort_count_nocommas",
        "state_id_x",
        "state_id_y",
    ],
...
transposed_CollegeRemediation = pd.melt(
    df_CollegeRemediation,
    id_vars=["state_id", "school_name", "cohort_count_nocommas"],
    value_vars=[
        "math_remed_rate",
        "english_remed_rate",
        "composite_remed_rate",
        "any_remed_rate",
    ],
)
