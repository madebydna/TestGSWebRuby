...
tx_preschools_df = pd.read_csv("TX_preschools.txt", sep="\t")
...
with open("DXT-3601_Deactivate_TX_Preschools.sql", "w") as sql_file:
    for line in tx_preschools_df.values:
        sql_file.write(
            "update _tx.school set active = 0 where id = {} and name = '{}' and level_code = '{}' and type = '{}' and active = {};\n".format(
                line[0], line[1].replace("'", "\\'"), line[3], line[4], line[5]
            )
        )
