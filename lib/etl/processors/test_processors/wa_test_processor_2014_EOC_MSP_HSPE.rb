...
class WATestProcessor2014EOC_MSP_HSPE < GS::ETL::TestProcessor
  ...
  source("2014_WA_EOC_MSP_HSPE_School.txt",[],col_sep:"\t") do |s|
      s.transform("fill entity level",Fill,{   
	 entity_level: 'school'
      })
  end 
  source("2014_WA_EOC_MSP_HSPE_District.txt",[],col_sep:"\t") do |s|
      s.transform("fill entity level",Fill,{   
	 entity_level: 'district'
      })
  end  
  source("2014_WA_EOC_MSP_HSPE_State.txt",[],col_sep:"\t") do |s|
      s.transform("fill entity level",Fill,{   
	 entity_level: 'state'
      })
  end
   ...
   shared do |s|
     ...
     .transform("Delelte rows with alt assessment data", DeleteRows, :test_data_type, 'WaasPort')
   ...
   def config_hash
   {
       ...
       file: 'wa/DXT-1781/wa.2014.1.public.charter.[level].txt',
       level: nil,
...
WATestProcessor2014EOC_MSP_HSPE.new(ARGV[0],max:nil).run
