...
module InsertStatement
  def self.build(row,source,load_type)
    if row[:entity_type] == 'source'
    	if load_type == 'test'
        	"insert into omni.data_sets (source_id,data_type_id,state,date_valid,configuration,notes,description) values ('#{source[:source_id]}',#{row[:data_type_id]},'#{source[:state].upcase}','#{row[:date_valid]}','none','#{::Mysql2::Client.escape(row[:notes])}','#{::Mysql2::Client.escape(row[:description])}');\n"
    	else
    		"insert into omni.data_sets (source_id,data_type_id,state,date_valid,configuration,notes,description) values ('#{source[:source_id]}',#{row[:data_type_id]},'#{source[:state].upcase}','#{row[:date_valid]}','none','#{::Mysql2::Client.escape(row[:notes])}',#{::Mysql2::Client.escape(row[:description])});\n"
    	end
