...
class WATestProcessor2015EOC_MSP_SBA < GS::ETL::TestProcessor
  ...
  source("2015_WA_EOC_MSP_SBA_School.txt",[],col_sep:"\t") do |s|
      s.transform("fill entity level",Fill,{   
	 entity_level: 'school'
      })
  end 
  source("2015_WA_EOC_MSP_SBA_District.txt",[],col_sep:"\t") do |s|
      s.transform("fill entity level",Fill,{   
	 entity_level: 'district'
      })
  end  
  source("2015_WA_EOC_MSP_SBA_State.txt",[],col_sep:"\t") do |s|
      s.transform("fill entity level",Fill,{   
	 entity_level: 'state'
      })
  end
   ...
   shared do |s|
     ...
     .transform("Delelte rows with alt assessment data", DeleteRows, :test_data_type, 'AIM')
   ...
   def config_hash
   {
       ...
       file: 'wa/DXT-1781/wa.2015.2.public.charter.[level].txt',
       level: nil,
...
WATestProcessor2015EOC_MSP_SBA.new(ARGV[0],max:nil).run
