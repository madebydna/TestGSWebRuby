...
module Components
  class Component
    attr_accessor :cache_data_reader, :data_type, :title, :type, :precision, :valid_breakdowns, :flagged,
                  :exact_breakdown_tags, :minimum_cutoff_year
    ...
    def low_income_to_percentages
      @_low_income_to_percentages ||= begin
        low_income = cache_data_reader.low_income_data.find {|ed| ed['breakdown'] == 'All students'}
        {
    ...
    def percentage_of_students_enrolled(breakdown)
      @_percentage_of_students_enrolled ||= begin
        hash = cache_data_reader.percentage_of_students(breakdown)
        {
