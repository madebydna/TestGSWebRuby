class TestScoresCaching::DistrictBreakdownsCacher <TestScoresCaching::DistrictTestScoresCacher
  ...
  def query_results
    @query_results ||= (
      results =
        (
          TestDataSet.fetch_test_scores(district, breakdown_id: 1) +
          TestDataSet.fetch_test_scores(district, grade: 'All')
        ).select do |result|
