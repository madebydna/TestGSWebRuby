shared_examples_for 'model with esp memberships' do
  ...
  describe '#is_esp_superuser' do
    ...
    let!(:esp_superuser_role) {FactoryBot.build(:role )}
    let!(:member_roles) {FactoryBot.build_list(:member_role,1,member_id: user.id,role_id:esp_superuser_role.id)}
  ...
  describe '#provisional_or_approved_osp_user' do
    ...
    context 'with an esp membership' do
      ...
      context 'with provisional esp membership' do
        let(:esp_membership) { FactoryBot.create(:esp_membership, :with_provisional_status, member_id: user.id) }
        it { is_expected.to be_provisional_or_approved_osp_user }
      end
      context 'with provisional esp membership' do
        let(:esp_membership) { FactoryBot.create(:esp_membership, :with_rejected_status, member_id: user.id) }
        it { is_expected.to_not be_provisional_or_approved_osp_user }
      end
    end
  end
