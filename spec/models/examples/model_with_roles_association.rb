shared_examples_for 'model with roles association' do
  ...
  describe '#has_role' do
    ...
    let!(:esp_superuser_role) {FactoryBot.build(:role,id:1 )}
    let!(:some_role) {FactoryBot.build(:role,id:2 )}
    let!(:member_roles) {FactoryBot.build_list(:member_role,1,member_id: user.id,role_id:2)}
