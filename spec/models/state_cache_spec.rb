...
describe StateCache do
  ...
  before do
    @state_cache = FactoryBot.create(:state_cache, state: 'ca', name: 'metrics')
  ...
  describe '.for_state' do
    it 'returns correct state cache record given a state' do
      expect(StateCache.for_state('metrics', 'CA')).to eq(@state_cache)
    end

    it 'returns nil if record not found' do
      expect(StateCache.for_state('metrics', 'KS')).to be_nil
    end
  end

  describe 'for_state_keys' do
    data_hash =  {"ca"=>{"state_attributes"=>{"growth_type"=>"Academic Progress Rating"}, "metrics"=>{}}, "ar"=>{"state_attributes"=>{"growth_type"=>"Student Progress Rating"}}}
    it 'returns the correct hash object when given state and keys' do
      expect(StateCache.for_state_keys(%w(state_attributes metrics), %w(ar ca))).to eq(data_hash)
    end
  end
