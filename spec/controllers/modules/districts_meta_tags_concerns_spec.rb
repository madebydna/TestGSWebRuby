...
describe DistrictsMetaTagsConcerns do
  ...
  context 'with a state, city, and district instance vars set' do
    before do
      ...
      subject.instance_variable_set(:@city, 'berkeley')
      subject.instance_variable_set(:@district, District.new(name: 'Berkeley Unified School District'))
    end

    it 'should return the correct title string' do
      expect(subject.send(:districts_show_title)).to eql("Berkeley Unified School District in Berkeley, CA | GreatSchools")
    end
  end
