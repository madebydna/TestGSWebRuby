        ...
        describe city_description do
          {
            ...
            it "should route a #{district_description}" do
              expect( get "/#{state}/#{city}/#{district}/schools/?oldsearch" ).to route_to('search#search', state: state, city: city.sub('%23', '#'), district_name: district.sub('%23', '#'), oldsearch: nil)
            end
          end
        end
      end
          ...
          describe city_description do
            {
              ...
              it "should route a #{district_description}" do
                expect( get "/#{state}/#{city}/#{district}/schools/" ).to route_to('search#search', state: state, city: city.sub('%23', '#'), district_name: district.sub('%23', '#'))
              end
            end
          end
        end
