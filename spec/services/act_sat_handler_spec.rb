...
describe SchoolProfiles::CollegeReadiness do
  describe "#handle_ACT_SAT_to_display!" do
    ...
    context 'given hash contains only ACT content' do
      let(:cv1) do 
        FactoryBot.build(:school_characteristics_value, school_value: 123, year: 2018, school_value_2018: 123, school_value_2014: 200)
      end

      let(:cv2) do 
        FactoryBot.build(:school_characteristics_value, school_value: 234, year: 2018, school_value_2018: 234, school_value_2014: 200)
      end

      let(:cv3) do 
        FactoryBot.build(:school_characteristics_value, school_value: 444, year: 2014, school_value_2014: 444)
      end
      ...
      it 'enforces latest year for ACT data type records' do
        hash = {
          act_sat_participation_dt => [cv1],
          act_score_dt => [cv2, cv3],
          act_sat_912_participation_dt => []
        }

        handler = ActSatHandler.new(hash)
        expect(handler.hash[act_score_dt].last.school_value).to_not be nil
        handler.handle_ACT_SAT_to_display!
        expect(handler.hash[act_score_dt].last.school_value).to be nil
      end

      it "ignores records for other subjects"
      it "ignores records for other CRDC breakdowns"
