...
describe 'Solr' do
  ...
  describe '#prepare_query_string' do
    it 'trims the query string' do
      expect(Solr::Solr.prepare_query_string('  anthony   ')).to eq('anthony')
    end
    it 'downcases the query string' do
      expect(Solr::Solr.prepare_query_string('Anthony')).to eq('anthony')
    end
    it 'pads commas with spaces in the query string' do
      expect(Solr::Solr.prepare_query_string('Anthony,Roy')).to eq('anthony, roy')
    end
    ...
    it 'does not alter the parameter in place' do
      input = '  foo  '
      expect(Solr::Solr.prepare_query_string(input)).to eq('foo')
  ...
  describe '#require_non_optional_words' do
    it 'requires regular words' do
      allow(Solr::Solr).to receive(:get_optional_words).and_return([])
      expect(Solr::Solr.require_non_optional_words('anthony roy')).to eq('+anthony +roy')
    end
    it 'does not require optional words' do
      allow(Solr::Solr).to receive(:get_optional_words).and_return(['district', 'high', 'school'])
      expect(Solr::Solr.require_non_optional_words('federer district high school')).to eq('+federer district high school')
    end
    ...
    it "treats both 'and' and '&' as optional" do
      expect(Solr::Solr.require_non_optional_words('School for Arts & Humanities and Sciences')).to eq('School +for +Arts & +Humanities and +Sciences')
    end
  end

  describe '#get_optional_words' do
    it 'includes a sample set of core optional words' do
      %w(school district charter preschool elementary middle high).each {|term| expect(Solr::Solr.get_optional_words).to include(term)}
    end
  end
end
