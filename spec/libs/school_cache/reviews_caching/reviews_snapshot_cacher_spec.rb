...
describe ReviewsCaching::ReviewsSnapshotCacher do
  ...
  let(:sample_reviews) do
    reviews = [
      FactoryBot.build(:five_star_review, created: now, answer_value: '5'),
      FactoryBot.build(:five_star_review, created: now, answer_value: '5')
    ]
  ...
  describe '#build_hash_for_cache' do
    let(:sample_reviews) do
      [
        FactoryBot.build(:teacher_effectiveness_review, answer_value: 'agree'),
        FactoryBot.build(:five_star_review, answer_value: 1),
        FactoryBot.build(:five_star_review, answer_value: 1),
        FactoryBot.build(:five_star_review, answer_value: 3),
        FactoryBot.build(:five_star_review, answer_value: 5),
        FactoryBot.build(:five_star_review, answer_value: 5),
        FactoryBot.build(:five_star_review, answer_value: nil)
      ]
    end
