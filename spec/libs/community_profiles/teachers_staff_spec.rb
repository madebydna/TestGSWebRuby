...
describe CommunityProfiles::TeachersStaff do
  ...
  describe "#main_staff_data" do
    ...
    it "ignores missing keys in the cache" do
      ...
      allow(cache_reader.decorated_district).to receive(:gsdata).and_return(data_with_missing_key)
    ...
    it "ignores missing values in the cache" do
      ...
      allow(cache_reader.decorated_district).to receive(:gsdata).and_return(data_with_missing_value)
    ...
    it "select latest year data for each data type" do
      ...
      allow(cache_reader.decorated_district).to receive(:gsdata).and_return(data_with_multiple_years)
    ...
    it "selects latest year for each data type" do
      data_with_multiple_years = {
        'Percent of Psychologist Staff' => [ 
          build(:full_time_other_staff, source_date_valid: "20160101 00:00:00"),
          build(:full_time_other_staff, source_date_valid: "20140101 00:00:00"),
          build(:part_time_other_staff, source_date_valid: "20160101 00:00:00"),
          build(:part_time_other_staff, source_date_valid: "20140101 00:00:00"),
          build(:no_other_staff, source_date_valid: "20160101 00:00:00"),
          build(:no_other_staff, source_date_valid: "20140101 00:00:00") ]
      }
      allow(cache_reader.decorated_district).to receive(:gsdata).and_return(data_with_multiple_years)
  ...
  describe "#sources_data" do
    ...
    it "is built from main and other staff data" do
      example_data = {
        'Percent of Nurse Staff' => [ 
          build(:full_time_other_staff, 
            source_name: "ABC Data Collection", 
            source_date_valid: "20170101 00:00:00"),
          build(:part_time_other_staff), 
          build(:no_other_staff)],
        'Ratio of students to full time teachers' => [ 
          build(:main_staff_hash, source_name: "CRDC", 
            source_date_valid: "20180101 00:00:00") 
        ]
      }

      allow(cache_reader.decorated_district).to receive(:gsdata).and_return(example_data)
