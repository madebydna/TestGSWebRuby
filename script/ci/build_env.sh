...
echo 'Debugging Github issues'
ping -c 5 www.github.com
traceroute -w 1 www.github.com

echo 'Setting up databases'
mysqldump -hstaging.greatschools.org -d -u${DB_USER} -p${DB_PASS} --databases gs_schooldb localized_profiles us_geo community api _ak _al _ar _az _ca _co \
...
mysqldump -hdev-gsdata.greatschools.org -d -u${DB_USER} -p${DB_PASS} --databases omni gsdata | sed -e 's/\(.*DATABASE.*\)`\(.*\)`/\1`\2_test`/;s/\(.*USE \)`\(.*\)`/\1`\2_test`/' | mysql -f -uroot
