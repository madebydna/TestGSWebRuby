...
export default class SectionNavigation extends React.Component {
  ...
  tabs() {
    ...
    return this.props.children.map((tab, index) => {
      const highlight = this.state.active == index;
      return [
  ...
  addDivider(index) {
    const last_item = this.props.children.length - 1;
    if (index != last_item) {
      return <span className="divider" key={`divider${index}`} />;
    }
  }
}
