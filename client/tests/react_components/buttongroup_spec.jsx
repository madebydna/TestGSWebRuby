...
describe('<ButtonGroup />', () => {
  ...
  const buttonGroup = () =>
      mount(<ButtonGroup options={options}
                         onSelect={onSelect}
  ...
  context('With four options', () => {
    it('Expect four buttons', () => expect(buttonGroup().find('button').length).to.eq(4));

    context('With elementary selected', () => {
      before(() => (activeOption = 'e'));

      it('Expect elementary to be selected', () => {
        let activeButton = buttonGroup().find('button.active');
      ...
      context('With allowDeselect false', () => {
        ...
        context('When trying to click on elementary', () => {
          it('Nothing happens', () => {
            buttonGroup().find('button.active').simulate('click');
        ...
        context('When trying to click on middle', () => {
          it('Triggers onSelect', () => {
            buttonGroup().find('button').not('.active').first().simulate('click');
    ...
    context('When choosing elementary', () => {
      ...
      it('should call the onSelect function with \'e\'', () => {
        buttonGroup().find('button').first().simulate('click');
